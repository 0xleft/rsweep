...
impl Sweeper {
    pub fn new(
        ...
        verbose: bool,
                                ...
                                // trust me
        let mut _ports = unsafe {
            PORTS.to_vec().clone().iter().map(|x| x.parse::<u32>().unwrap()).collect()
        };
        if ports != "all" {
