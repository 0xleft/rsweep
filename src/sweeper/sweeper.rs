...
impl Sweeper {
    pub fn new(
        ...
        verbose: bool,
                                ...
                                // trust me
        let mut _ports = (1..65535).collect::<Vec<u32>>();
        if ports != "all" && ports != "top" {
            _ports = ports.split(",").map(|x| x.parse::<u32>().unwrap()).collect();
        } else if ports == "top" {
            _ports = unsafe {
                MOST_USED_1000_PORTS.to_vec().clone().iter().map(|x| x.parse::<u32>().unwrap()).collect()
            };
        }
        let ports = _ports;
